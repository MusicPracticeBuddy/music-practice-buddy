...
INSERT INTO session (musician_id, session_template_id, status, assigned_at, started_at, ended_at)
VALUES
    ((SELECT id FROM musician WHERE is_admin LIMIT 1), (SELECT id FROM session_template WHERE name = 'Daily Brass Practice'), 'COMPLETED', '2026-08-23 08:00:00-04', '2026-08-23 08:05:00-04', '2026-08-23 08:55:00-04'),
    ((SELECT id FROM musician WHERE is_admin LIMIT 1), (SELECT id FROM session_template WHERE name = 'Daily Brass Practice'), 'IN_PROGRESS', '2026-08-25 08:00:00-04', '2026-08-25 08:05:00-04', NULL),
    ((SELECT id FROM musician WHERE is_admin LIMIT 1), (SELECT id FROM session_template WHERE name = 'Short Practice Session'), 'PLANNED', '2026-08-26 08:00:00-04', NULL, NULL);
